...
export function i18n() {
    var self = this,
        jsonHost = 'http://localhost/react-i18n/json-api',
    ...
    console.log('Language: ' + cookie.get("i18n"));

    fetch(jsonHost + "/config.json", { cache: "no-cache" }).then(function (response) {
    ...
    self.loadCollection = function (collection_name, cIndex, resolve, reject) {
        ...
        if (typeof self.language.collections[collection_name] !== 'undefined' && typeof self.language.collections[collection_name].loaded === 'undefined') {
            if ( self.language.collections[collection_name].loaded !== false ) {
                ...
                fetch(jsonHost + '/' + self.language.index + "/" + collection_name + ".json?_=" + self.language.collections[collection_name].mtime, { cache: "force-cache" }).then(function(response) {
                    return response.json();
                }).then(function(response) {
    ...
    self.updateIndex = function (cIndex, tranlation) {
        var cIndexData = self.cIndexData(cIndex);
        if (cIndexData) {
            var data = {
                command: 'update',
            ...
            fetch(jsonHost + "/api.php", {
                ...
                self.language.collections[cIndexData.collection].data[response.data.index] = response.data.tranlation;
